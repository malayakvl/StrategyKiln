...
const Threats2OpportunitiesForm: React.FC<Props> = ({
  props,
  num,
  counter,
  customError,
  ...
  const handleDataAdd = (data: any, index: number) => {
    if (data?.length) {
      ...
      dispatch(setThreats2OpportunitiesDisplayAction(tmpServiceList));
  ...
  useEffect(() => {
    setDataError(false);
  }, [props.values[`threats2Opportunities_${num}_description`]]);
  ...
  return (
    <div className="position-relative threats2Opportunities-block-form">
...
export { Threats2OpportunitiesForm };
