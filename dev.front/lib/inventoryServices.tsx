...
export function prepareConfigValues(
  values: any,
  selectedColors: any,
  selectedSizes: any
) {
    ...
    for (let i = 0; i < _confColors.length; i++) {
      for (let j = 0; j < _confSizes.length; j++) {
        ...
        configData.push({
          price:
            values[
              `configurePrice_${_confColors[i].label}_${_confSizes[j].label}`
            ],
          qty: values[
            `configureQty_${_confColors[i].label}_${_confSizes[j].label}`
          ],
          sku: values[
            `configureSKU_${_confColors[i].label}_${_confSizes[j].label}`
          ],
        });
      }
    }
